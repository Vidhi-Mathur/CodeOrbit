...
import { useGithubCalendarQuery } from "@/hooks/useGitHub"
...
export const GitHubCalendar = ({ contributions, username }: GitHubCalendarProps) => {
    const allYears = useMemo(() => {
        return [...(contributions?.activeYears || [])].sort((a, b) => b - a)
    }, [contributions])

    const [selectedYear, setSelectedYear] = useState<number>(allYears[0])

    const { data, isFetching } = useGithubCalendarQuery(username, selectedYear, contributions)

    const selectedYearData = contributions?.contributionCalendarResponse?.[selectedYear] || data?.calendarResponse?.contributionCalendarResponse?.[selectedYear]
    ...
    return (
        <div className="bg-white rounded-lg sm:rounded-lg md:rounded-lg lg:rounded-xl shadow-sm border relative m-1 sm:m-1.5 md:m-0 lg:m-2">
            <div className="mb-3 sm:mb-3 md:mb-3 lg:mb-4">
                ...
                {isFetching && !selectedYearData && <ShimmerInnerCalendar />}
                {!isFetching && !selectedYearData && (
