...
export async function GET(req: NextRequest, { params }: { params: Promise<{ github_username: string }> }) {
  ...
  try {
    ...
    const pinnedRepos = (user.pinnedItems?.nodes || []).map(extractedRepos);
    const recentRepos = (user.recentRepositories?.nodes || []).map(extractedRepos);

    const reposMap = new Map<string, GitHubRepoInterface>();
    pinnedRepos.forEach(repo => reposMap.set(repo.name, repo));
    ...
    // 2. Determine active years (based on createdAt)
    ...
    // 3. Fetch contribution calendars per year
    const calendarPromises = activeYears.map((year) => {
      const now = new Date();
const from = `${year}-01-01T00:00:00Z`;
const to =
  year === now.getFullYear()
    ? now.toISOString() // only up to today if current year
    ...
    const calendarMap: Record<number, {
    ...
    return new Response(
      JSON.stringify({ profileResponse, reposResponse, calendarResponse }),
      { status: 200 }
    );
